...
void updateBPM(float currentTime, float deltaTime) {
    if (isRunning) {
        ...
        float targetBPM = baseBPM + (totalRunningTime * bpmIncreaseRate);
...
void drawEKGGraph(float deltaTime) {
    ...
    // Clamp bpmFactor za prikaz grafika, ali ne i za stvarni BPM
    float displayBPM = currentBPM;
    if (displayBPM > 220.0f) displayBPM = 220.0f; // Za grafik, koristi max 220

    float bpmFactor = (displayBPM - 60.0f) / 160.0f;
